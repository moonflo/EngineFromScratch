...
bool InitializeOpenGL(HWND hwnd, int screenWidth, int screenHeight,
    ...
    result = wglChoosePixelFormatARB(g_deviceContext, attributeListInt, NULL, 1,
                                     pixelFormat, &formatCount);
    if (result != 1) {
        return false;
    }

    // If the video card/display can handle our desired pixel format then we set
    // it as the current one.
    result =
        SetPixelFormat(g_deviceContext, pixelFormat[0], &pixelFormatDescriptor);
    if (result != 1) {
        return false;
    }
    ...
    g_renderingContext =
        wglCreateContextAttribsARB(g_deviceContext, 0, attributeList);
    ...
    BuildPerspectiveFovLHMatrix(g_projectionMatrix, fieldOfView, screenAspect,
                                screenNear, screenDepth);

    // Get the name of the video card.
    vendorString = (char *)glGetString(GL_VENDOR);
    rendererString = (char *)glGetString(GL_RENDERER);
    // Store the video card name in a class member variable so it can be
    // retrieved later.
...
bool InitializeExtensions(HWND hwnd) {
    ...
    // Initialize the OpenGL extensions needed for this application.  Note that
    // a temporary rendering context was needed to do so.
    result = LoadExtensionList();
    if (!result) {
        return false;
    }

    // Release the temporary rendering context now that the extensions have been
    // loaded.
    wglMakeCurrent(NULL, NULL);
...
void OutputShaderErrorMessage(HWND hwnd, unsigned int shaderId,
...
void OutputLinkerErrorMessage(HWND hwnd, unsigned int programId) {
    ...
    MessageBox(
        hwnd,
        _T("Error compiling linker.  Check linker-error.txt for message."),
        _T("Linker Error"), MB_OK);
}
...
bool InitializeShader(HWND hwnd, const char *vsFilename,
...
bool InitializeBuffers() {
    ...
    uint16_t indices[] = {1, 2, 3, 3, 2, 6, 6, 7, 3, 3, 0, 1, 0, 3, 7, 7, 6, 4,
                          4, 6, 5, 0, 7, 4, 1, 0, 4, 1, 4, 5, 2, 1, 5, 2, 5, 6};
    ...
    glBufferData(GL_ARRAY_BUFFER, g_vertexCount * sizeof(VertexType), vertices,
                 GL_STATIC_DRAW);
    ...
    // Specify the location and format of the color portion of the vertex
    // buffer.
    glBindBuffer(GL_ARRAY_BUFFER, g_vertexBufferId);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, g_indexCount * sizeof(uint16_t),
