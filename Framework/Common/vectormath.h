...
void MatrixRotationYawPitchRoll(float* matrix, float yaw, float pitch,
                                float roll) {
...
void TransformCoord(VectorType& vector, float* matrix) {
    ...
    x = (vector.x * matrix[0]) + (vector.y * matrix[3]) +
        (vector.z * matrix[6]);
    y = (vector.x * matrix[1]) + (vector.y * matrix[4]) +
        (vector.z * matrix[7]);
    z = (vector.x * matrix[2]) + (vector.y * matrix[5]) +
        (vector.z * matrix[8]);
...
void BuildPerspectiveFovLHMatrix(float* matrix, float fieldOfView,
