 ...
 * @LastEditTime: 2023-07-10 18:36:40
...
ENUM(VertexDataType){
    ...
    kVertexDataTypeDouble3 = "DUB3"_i32, kVertexDataTypeDouble4 = "DUB3"_i32};
...
std::vector<std::string> sPrimitiveNames = {"None",
                                            "PointList",
                                            "LineList",
                                            "LIneStrip",
                                            "TriList",
...
typedef enum _PrimitiveType : int32_t {
    ...
    kPrimitiveTypeTriList =
        "TLST"_i32,  ///< For N>=0, vertices [N*3+0, N*3+1, N*3+2] render a triangle.
    ...
    kPrimitiveTypeLineListAdjacency =
        "LLSA"_i32,  ///< For N>=0, vertices [N*4..N*4+3] render a line from [1, 2]. Lines [0, 1] and [2, 3] are adjacent to the rendered line.
    kPrimitiveTypeLineStripAdjacency =
        "LSTA"_i32,  ///< For N>=0, vertices [N+1, N+2] render a line. Lines [N, N+1] and [N+2, N+3] are adjacent to the rendered line.
    kPrimitiveTypeTriListAdjacency =
        "TLSA"_i32,  ///< For N>=0, vertices [N*6..N*6+5] render a triangle from [0, 2, 4]. Triangles [0, 1, 2] [4, 2, 3] and [5, 0, 4] are adjacent to the rendered triangle.
    kPrimitiveTypeTriStripAdjacency =
        "TSTA"_i32,  ///< For N>=0, vertices [N*4..N*4+6] render a triangle from [0, 2, 4] and [4, 2, 6]. Odd vertices Nodd form adjacent triangles with indices min(Nodd+1,Nlast) and max(Nodd-3,Nfirst).
    kPrimitiveTypeRectList =
        "RLST"_i32,  ///< For N>=0, vertices [N*3+0, N*3+1, N*3+2] render a screen-aligned rectangle. 0 is upper-left, 1 is upper-right, and 2 is the lower-left corner.
...
std::ostream& operator<<(std::ostream& out, PrimitiveType type) {
    ...
    if (typeNum < 0 || typeNum > 16)
        typeNum = 0;
        //out << "int:" << int(type);
    out << sPrimitiveNames[typeNum];
    return out;
}
...
struct ParameterValueMap {
    ...
    friend std::ostream& operator<<(std::ostream& out,
        ...
        if (obj.ValueMap) {
            out << "Parameter Map: " << *obj.ValueMap;
        }
   ...
   public:
    ...
    friend std::ostream& operator<<(std::ostream& out,
        ...
        for (decltype(count) i = 0; i < count; i++) {
            out << "This Geometry SO's mesh[" << i << "]:\n"
                << *obj.m_Mesh[i] << std::endl;
        }
        return out;
    }
};
...
class SceneObjectLight : public BaseSceneObject {
   ...
   protected:
    ...
    SceneObjectLight(Color&& color = Vector4f(1.0f), float intensity = 10.0f,
...
class SceneObjectSpotLight : public SceneObjectLight {
   ...
   public:
    SceneObjectSpotLight(Color&& color = Vector4f(1.0f),
...
class SceneObjectPerspectiveCamera : public SceneObjectCamera {
   ...
   public:
    SceneObjectPerspectiveCamera(float aspect = 16.0f / 9.0f,
                                 ...
                                 float far_clip = 100.0f, float fov = PI / 2.0)
...
class SceneObjectTransform {
   ...
   public:
    ...
    SceneObjectTransform(const Matrix4X4f& matrix,
