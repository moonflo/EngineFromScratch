    ...
    virtual Image Parse(const Buffer& buf) {
        ...
        if (pFileHeader->Signature ==
            ...
            while (pData < pDataEnd) {
                const PNG_CHUNK_HEADER* pChunkHeader =
                ...
                switch (type) {
                    case PNG_CHUNK_TYPE::IHDR: {
                        ...
                        std::cout << "----------------------------"
                                  << std::endl;
                        const PNG_IHDR_HEADER* pIHDRHeader =
                        ...
                        img.Height = m_Height;
                        img.bitcount =
                            32;  // currently we fixed at RGBA for rendering
                        ...
                        std::cout << "Bit Depth: " << (int)m_BitDepth
                                  << std::endl;
                        std::cout << "Color Type: " << (int)m_ColorType
                                  << std::endl;
                        std::cout << "Compression Method: "
                                  << (int)m_CompressionMethod << std::endl;
                        std::cout << "Filter Method: " << (int)m_FilterMethod
                                  << std::endl;
                        std::cout
                    ...
                    case PNG_CHUNK_TYPE::PLTE: {
                        ...
                        for (auto i = 0;
                             i < endian_net_unsigned_int(pPLTEHeader->Length) /
                    ...
                    case PNG_CHUNK_TYPE::IDAT: {
                        ...
                        std::cout << "Compressed Data Length: "
                                  << compressed_data_size << std::endl;
                        ...
                        const uint8_t* pIn =
                            pData +
                            sizeof(
                                PNG_CHUNK_HEADER);  // point to the start of the input data buffer
                        uint8_t* pOut = reinterpret_cast<uint8_t*>(
                            img.data);  // point to the start of the input data buffer
                            ...
                            do {
                                ...
                                while (p - pDecompressedBuffer <
                                    ...
                                    if (current_col == -1) {
                                        ...
                                        A = (current_col < m_BytesPerPixel)
                                                ? 0
                                                : *(pOut +
                                                    img.pitch * current_row +
                                                    current_col -
                                                    m_BytesPerPixel);
                ...
                pData += endian_net_unsigned_int(pChunkHeader->Length) +
                         sizeof(PNG_CHUNK_HEADER) + 4 /* length of CRC */;
            }
        } else {
