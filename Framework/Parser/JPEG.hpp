...
#endif
                block[i][0][0] +=
                    1024.0f;  // level shift. same as +128 to each element after IDCT
                ...
                switch (endian_net_unsigned_int(pSegmentHeader->Marker)) {
                    ...
                    case 0xFFC2: {
                        if (endian_net_unsigned_int(pSegmentHeader->Marker) ==
                            0xFFC0)
                            std::cout << "Start Of Frame0 (baseline DCT)"
                                      << std::endl;
                        else
                            std::cout << "Start Of Frame2 (progressive DCT)"
                                      << std::endl;

                        std::cout << "----------------------------"
                                  << std::endl;

                        const FRAME_HEADER* pFrameHeader =
                        ...
                        std::cout << "Sample Precision: " << m_nSamplePrecision
                                  << std::endl;
                        std::cout << "Num of Lines: " << m_nLines << std::endl;
                        std::cout
                            << "Num of Samples per Line: " << m_nSamplesPerLine
                            << std::endl;
                        std::cout << "Num of Components In Frame: "
                                  << m_nComponentsInFrame << std::endl;
                        std::cout << "Total MCU count: " << mcu_count
                                  << std::endl;
                        ...
                        img.data_size =
                            img.pitch * mcu_count_y * 8 * (img.bitcount >> 3);
...
#ifdef DUMP_DETAILS
                            std::cout << m_tableQuantization
                                    [pQtable->DestinationIdentifier()];
#endif

                            size_t processed_length =
                                sizeof(QUANTIZATION_TABLE_SPEC) +
                                64 * (pQtable->ElementPrecision() + 1);
                            pTmp += processed_length;
                            segmentLength -= processed_length;
                        }
                        pData +=
                    ...
                    case 0xFFDD: {
                        ...
                        RESTART_INTERVAL_DEF* pRestartHeader =
                            (RESTART_INTERVAL_DEF*)pData;
                        m_nRestartInterval = endian_net_unsigned_int(
                            (uint16_t)pRestartHeader->RestartInterval);
                        std::cout << "Restart interval: " << m_nRestartInterval
                                  << std::endl;
                        pData +=
                            endian_net_unsigned_int(pSegmentHeader->Length) +
                            2 /* length of marker */;
                    ...
                    case 0xFFDA: {
                        ...
                        std::cout << "Image Conponents in Scan: "
                            ...
                            *(uint32_t*)pApp0->Identifier)) {
                            case "JFIF\0"_u32: {
                                ...
                                if (pJfifApp0->Xthumbnail &&
                                    ...
                                    std::cout << "Thumbnail Dimesions [w*h]: "
                                              << (uint16_t)pJfifApp0->Xthumbnail
                                              << "*"
                                              << (uint16_t)pJfifApp0->Ythumbnail
                                              << std::endl;
                                } else {
                                    std::cout << "No thumbnail defined in "
                                                 "JFIF-APP0 segment!"
                                              << std::endl;
                                }
                            } break;
                            case "JFXX\0"_u32: {
                                const JFXX_APP0* pJfxxApp0 =
                            ...
                            default:
                                std::cout << "Ignor Unrecognized APP0 segment."
                                          << std::endl;
                        }
                        pData +=
                            endian_net_unsigned_int(pSegmentHeader->Length) +
