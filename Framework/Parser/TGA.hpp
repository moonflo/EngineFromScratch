...
class TgaParser : implements ImageParser {
   public:
    virtual Image Parse(const Buffer& buf) {
...
#ifdef DEBUG
        std::cout << "ID Length: " << (uint16_t)pFileHeader->IDLength
                  << std::endl;
        ...
        std::cout << "Image Pixel Depth: " << (uint16_t)pixel_depth
                  << std::endl;
        std::cout << "Image Alpha Depth: " << (uint16_t)alpha_depth
                  << std::endl;
#endif
        // skip Image ID
            ...
            for (auto j = 0; j < img.Width; j++) {
                switch (pixel_depth) {
                    case 15: {
                        ...
                        pData += 2;
                        *(pOut + img.pitch * i + j * 4) =
                            ((color & 0x7C00) >> 10);  // R
                        *(pOut + img.pitch * i + j * 4 + 1) =
                            ((color & 0x03E0) >> 5);  // G
                        *(pOut + img.pitch * i + j * 4 + 2) =
                            (color & 0x001F);                        // B
                    ...
                    case 16: {
                        ...
                        *(pOut + img.pitch * i + j * 4 + 2) =
                            (color & 0x001F);  // B
                        *(pOut + img.pitch * i + j * 4 + 3) =
                            ((color & 0x8000) ? 0xFF : 0x00);  // A
                    } break;
                    case 24: {
